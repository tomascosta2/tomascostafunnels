...
// ✅ El default export de una page en App Router
//    SOLO puede recibir { params?, searchParams? }
type PageProps = {
  searchParams?: {
    nombre?: string;       // opcional: ?nombre=Tomás%20Costa
    wa?: string;           // opcional: ?wa=+542616841853
    msg?: string;          // opcional: ?msg=Hola%20confirmo%20mi%20asistencia
  };
};

export default function ThankYouPage({ searchParams }: PageProps) {
  // Valores por defecto + overrides desde querystring
  const nombre = searchParams?.nombre ? decodeURIComponent(searchParams.nombre) : "¡Ya casi estamos!";
  const whatsappNumber = (searchParams?.wa ?? "+542616841853").replace(/\s+/g, "");
  const whatsappMessage =
    searchParams?.msg
      ? decodeURIComponent(searchParams.msg)
      : "Hola Tomi, confirmo mi asistencia a la llamada";
    ...
    <div className="max-w-[760px] mx-auto px-4 py-[60px] bg-white text-black">
      ...
      <div className="space-y-3 mb-6">
        ...
        <label className="flex items-start gap-3">
          <input type="checkbox" className="mt-1 size-4" checked={c1} onChange={(e) => setC1(e.target.checked)} />
          <span>Voy a estar en un lugar tranquilo, sin interrupciones.</span>
        </label>

        <label className="flex items-start gap-3">
          <input type="checkbox" className="mt-1 size-4" checked={c2} onChange={(e) => setC2(e.target.checked)} />
          <span>Me comprometo a llegar a tiempo (respeto el cupo y la agenda).</span>
        </label>

        <label className="flex items-start gap-3">
          <input type="checkbox" className="mt-1 size-4" checked={c3} onChange={(e) => setC3(e.target.checked)} />
      ...
      <a
        ...
        href={c1 && c2 && c3 ? waHref : undefined}
        target="_blank"
        rel="noopener noreferrer"
        aria-disabled={!(c1 && c2 && c3)}
      >
